...
class _MapScreenState extends State<MapScreen> {
  ...
  Future<void> _onDiscoverChallenges() async {
    if (_isDiscovering) return;
    setState(() => _isDiscovering = true);
    try {
      final result = await GenkitService().discoverChallengesFromWeb();
      if (!mounted) return;
      ...
      if (success && count > 0) {
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text('$message Refreshing map...')),
          );
        }
      } else {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text(success ? message : 'Discovery failed: $message')),
          );
        }
      }
    } catch (e) {
    ...
    return Scaffold(
      ...
      body: Stack(
        children: [
          ...
          // Discover Challenges from HK Gov Tourism button
          Positioned(
            ...
            child: Material(
              ...
              borderRadius: BorderRadius.circular(12),
              child: InkWell(
                onTap: _isDiscovering ? null : _onDiscoverChallenges,
                borderRadius: BorderRadius.circular(12),
                child: Container(
