...
TOURISM_SITES = [
    # HK Government Tourism Commission – official recommendations
    "https://www.tourism.gov.hk/",
    "https://www.tourism.gov.hk/tourism-projects.php",  # Tourist Hotspots
    # HK Tourism Board (linked from gov site)
...
def _build_site_prompts():
    return "\n\n".join(
        f"SITE {i + 1}:\n"
        f"1. init_session with session_name \"site-{i + 1}-browse\"\n"
        f"2. navigate to {url}\n"
        f"3. Use get_text with selector \"body\" to read the page content\n"
        f"4. close the session"
        for i, url in enumerate(TOURISM_SITES)
    )
...
After browsing all {len(TOURISM_SITES)} sites, generate 10-15 challenge objects from the attractions you found.
Prioritize official recommendations from the Tourism Commission (tourism.gov.hk) and HKTB.
